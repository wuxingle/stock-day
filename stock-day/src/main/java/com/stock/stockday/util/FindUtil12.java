...
public class FindUtil12 {
    ...
    public static boolean duoTuXiangShangDapan2(List<StockDayAll> stockDayAllList, List<ScienceEntity> scienceEntityList, List<StockDayAll> stockDayAllListDaPan1, List<ScienceEntity> scienceEntityListDaPan) {
        ...
        try {
            boolean result = shangReset20(stockDayAllListDaPan1, scienceEntityListDaPan);
            if (result) {
                //均线聚拢,开始多天向上
    ...
    public static boolean shangReset20(List<StockDayAll> stockDayAllList, List<ScienceEntity> scienceEntityList) {
        if (stockDayAllList == null || stockDayAllList.size() == 0) {
            return false;
        }
        if (scienceEntityList == null || scienceEntityList.size() == 0) {
        ...
        try {
            if (stockDayAllList.get(0).getClosingPrice() != null && stockDayAllList.get(1).getClosingPrice() != null && stockDayAllList.get(0).getFoot() != null && stockDayAllList.get(0).getOpeningPrice() != null) {
                //前几天在5天线上 近5天下降
                ...
                if (xia5.compareTo(new BigDecimal(-3)) <= 0 || xia4.compareTo(new BigDecimal(-2)) <= 0 || xia3.compareTo(new BigDecimal(-1.0)) <= 0 || xia2.compareTo(new BigDecimal(-0.5)) <= 0) {
                    if (stockDayAllList.get(0).getClosingPrice().compareTo(stockDayAllList.get(5).getClosingPrice()) < 0) {
                        //今天阳线
                        if (stockDayAllList.get(0).getClosingPrice().compareTo(stockDayAllList.get(0).getOpeningPrice()) > 0) {
                            return true;
